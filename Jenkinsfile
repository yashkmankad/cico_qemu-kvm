  ...
  stages {
      ...
      steps {
        ...
        wrap([$class: 'AnsiColorBuildWrapper', 'colorMapName': 'xterm']) {
            sh '''
              ...
              # get node from duffy
              export CICO_API_KEY=$(cat /home/qemu-kvm/duffy.key); cico node get --arch x86_64 --release 7 --count 1
    ...
    stage('Install QEMU') {
      steps {
            ...
            ssh $SSH_OPTIONS \
            root@$CENTOS_NODE "git clone git://git.qemu-project.org/qemu.git && cd qemu && git submodule init && git submodule update --recursive && ./configure && make"
            '''
            }
          }
    ...
    stage('Install Avocado') {
      steps {
            ...
            if [ "${AVOCADO_VERSION}" != "master" ]; then
              ...
              ssh $SSH_OPTIONS \
              root@$CENTOS_NODE "yum install -y python-avocado-${AVOCADO_VERSION} avocado-plugins-vt-${AVOCADO_VERSION} python-avocado-plugins-loader-yaml-${AVOCADO_VERSION}"
            else
